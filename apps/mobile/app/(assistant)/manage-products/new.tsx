...
export default function AddProductScreen() {
    ...
    const handleCreate = async () => {
        if (!name || !price || categories.length === 0) {
            setDialog({ visible: true, title: 'Error', message: 'Please fill in Name, Price, and at least one Category', isError: true });
            return;
        }

        if (salesPrice && startDate && endDate) {
            if (new Date(startDate) > new Date(endDate)) {
                setDialog({ visible: true, title: 'Error', message: 'Sales End Date must be after Start Date', isError: true });
                return;
            }
        }
        ...
        try {
            ...
            setDialog({ visible: true, title: 'Success', message: 'Product created successfully', isError: false });
            router.back();
        } catch (e) {
            setDialog({ visible: true, title: 'Error', message: 'Failed to create product', isError: true });
        } finally {
            setLoading(false);
        }
    };
